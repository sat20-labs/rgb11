...
| 官方 Wallet schema | NIA、IFA、CFA、UDA contract import、issuance、transfer validation | 四份官方 contract fixture；四 schema genesis 与 confidential transition 差分 |
...
| L1 发行 | Wallet SDK 选择本钱包已确认 plain Bitcoin UTXO，构造并本地验证 contract | 四 schema 发行、余额投影、`reason=rgb` lock 测试 |
| L1 收发 | Invoice、Opret witness、PSBT、Consignment、Relay、ACK gate、广播 | 官方 NIA Relay/ACK E2E；NIA/IFA/CFA/UDA 发行后 send/receive E2E |
| Tapret / Opret | Go Engine 同时验证两种 carrier binding；新转账遵循冻结版 Opret 默认值 | anchor、Tapret key-path signing、Opret witness tests |
...
| PWA | 统一 L1 列表、四 schema 发行、导入、Invoice、收发、备份/恢复、proof 详情、transfer monitor | WASM export smoke、`vue-tsc`、production build |
